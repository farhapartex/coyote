...
| `coyote worker` | Run background jobs as their own process |
...
| `--concurrency=N` | `worker` | Run N jobs at once instead of `Jobs.Workers` |
| `--queues=A,B` | `worker` | Take only from these queues instead of `Jobs.Queues` |
